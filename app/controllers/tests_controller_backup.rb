class TestsController < ApplicationController
  ...
  before_action :set_test_group, only: [:create]
  ...
  # POST /tests/:id/toggle_core
  def toggle_core
    @test.update(is_core: !@test.is_core)
    render json: { test: test_json(@test) }
  end
  ...
  def stats
    render json: {
      ...
      core_tests: Test.core.count,
      tests_by_group: TestGroup.includes(:tests).map { |g|
        {
          group_name: g.name,
          total: g.tests.count,
          active: g.tests.active.count,
          core: g.tests.active.core.count
        }
      }
    }
  end
